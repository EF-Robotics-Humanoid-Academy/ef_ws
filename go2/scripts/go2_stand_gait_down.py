...
class StandGaitDown:
    def __init__(self):
        ...
        # Gait params (conservative).
        self.period = 2.8
        self.swing_ratio = 0.22
        self.step_amp = 0.5
        self.stance_amp = 0.28
        self.lift_amp = 1
        self.swing_abduct = 0.5

        # IMU-based stabilization and forward bias.
        self.k_roll = 0.15
        self.k_pitch = 0.15
    ...
    def _gait_targets(self, t):
        ...
        for leg in self.legs:
            ...
            if swinging:
                ...
                q[1] += forward + 1.2 * lift
                q[2] += -4.2 * lift
                if leg in ("FL", "RL"):
                    q[0] += self.swing_abduct
                else:
                    q[0] -= self.swing_abduct
            else:
            ...
            if swing_leg is not None and not swinging:
                if swing_leg in ("FL", "RL"):
                    if leg in ("FR", "RR"):
                        roll_term += self.support_roll_bias
                    else:
                        roll_term -= self.support_roll_bias
                else:
                    if leg in ("FL", "RL"):
            ...
            if leg in ("FL", "FR"):
                q[1] -= pitch_base + self.forward_bias
            else:
                q[1] += pitch_base + self.forward_bias
